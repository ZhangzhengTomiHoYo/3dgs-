...
if __name__ == '__main__':
    origin = np.array([[2., 0., -2.], [0., 2., -2.], [-2., 0., -2.]])
    lst = []
    n = 0.1
    for _ in range(5):
        origin[0][1] += n
        print(origin)
        lst.append(origin.copy())
    for _ in range(10):
        origin[0][1] -= n
        lst.append(origin.copy())
    for _ in range(10):
    ...
    for i, gs_pos in enumerate(lst):
        out_color = create_lst(gs_pos)
        out_color = np.clip(out_color, 0, 1)
        plt.imsave(f"output/{i:03d}.png", out_color)
